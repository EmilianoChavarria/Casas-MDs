...
| **Panel del co-anfitrión** (5.13) | Filtro por pertenencia en **cada consulta** de `/host/*`, igual que el panel del guía y por el mismo motivo. Sus reservas se filtran por el `cohost_user_id` congelado, no por el pivote. **No recibe correo ni teléfono del huésped** —con ellos cierra la siguiente reserva por fuera— ni la comisión del procesador, ni nada de casas ajenas. `isStaff()` devuelve falso para él, así que `EnsureUserIsAdmin` ya lo rechaza del panel sin añadir nada |
