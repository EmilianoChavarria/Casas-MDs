...
- **DRY/KISS/YAGNI:** no construir multi-tenant hasta que exista un requerimiento real; empezar simple y extraer abstracciones solo cuando se repitan 3+ veces. En el frontend esa regla ya se disparó: el prototipo duplica el componente `Input` en 4 páginas y `Row` en 3 — al portar se unifican en `components/ui/` (sección 17.6).
  ⚠️ **Sobre multi-tenant:** el requerimiento llegó — hay dueños externos (co-anfitriones) que ven sus propias casas dentro del sistema. **Se resolvió con un rol y un filtro por pertenencia, no con multi-tenant**: sin `tenant_id`, sin datos duplicados y sin bases separadas, porque el administrador sigue viéndolo todo y el inventario sigue siendo uno solo. Multi-tenant resolvería un problema que no existe (aislar empresas que compiten entre sí) y cobraría su precio en cada consulta del sistema. Ver 1.1 y 5.13.
