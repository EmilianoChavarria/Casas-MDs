...
5. Cambiar los **nameservers** del dominio (en el registrador, ver `../11-dominio/`) a los que indique Cloudflare (ej. `ana.ns.cloudflare.com`, `bob.ns.cloudflare.com`).
...
7. Configurar registros DNS necesarios:
   ...
   ```
8. **SSL/TLS → Overview** → seleccionar modo **Full (Strict)** (requiere certificado válido en el origen, ver Certbot en `../01-hosting-vps/`, o usar Cloudflare Origin Certificate).
